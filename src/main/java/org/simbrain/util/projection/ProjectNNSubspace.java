...
public class ProjectNNSubspace extends ProjectionMethod {
    ...
    public void project() {
        ...
        switch (numPoints) { // assumes numPoints is a positive integer
            ...
            default: {
                int[] neighbors = projector.getUpstairs().getKNearestNeighbors(3, lastAddedUpstairs); // KNN returning identical points
                point1Index = neighbors[0];
                point2Index = neighbors[1];
                ...
                for (int k = 0; k < point1Up.getDimension(); ++k) {
                    norm1 += ((point2Up.get(k) - point1Up.get(k)) * (point2Up.get(k) - point1Up.get(k)));
                }
                ...
                for (int k = 0; k < point1Down.getDimension(); ++k) {
                    norm1 += ((point2Down.get(k) - point1Down.get(k)) * (point2Down.get(k) - point1Down.get(k)));
                }
