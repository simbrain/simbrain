...
val spikingNeuron = newSim {
    ...
    addSidebarInfo(
        ...
        This simulation allows you to explore the behavior of a [spiking neuron model](https://docs.simbrain.net/docs/network/spikingneurons.html), which responds to inputs in a realistic
        brain-inspired way. You can see how the neuron's behavior can be influenced by inputs, and how spikes trigger a post 
        synaptic response or [PSR](https://en.wikipedia.org/wiki/Postsynaptic_potential).  
        
        Spiking neurons fire only when their internal signal becomes large enough. In this simulation, you provide input to a neuron, which then spikes and affects another neuron. 
        The influence of each spike fades over time, mimicking how real brain cells communicate.
        ...
        This setup uses the [Izhikevich neuron model](https://docs.simbrain.net/docs/network/neurons/izhikevich.html).
        which is able to capture a wide variety of biologically realistic firing patterns. The Izhikevich model combines 
        nonlinear membrane potential dynamics with a recovery variable to emulate diverse firing behaviors using just four parameters (A, B, C, D).
        It integrates user-driven input and exhibits dynamic spiking governed by a 2-variable system. 
                 
        When a spike occurs, A [jump-and-decay](https://docs.simbrain.net/docs/network/spikeresponders/jumpdecay.html) 
        spike responder simulates how spikes affect downstream neurons, creating smooth post-synaptic potentials.
        Spike transmission is modeled by a spike responder, which applies a transient signal to the post-synaptic neuron.
        ...
        - **Change the Izhikevich parameters**: Double-click the `Spiking` neuron and modify A, B, C, D. Use the [Izhikevich documentation](https://docs.simbrain.net/docs/network/neurons/izhikevich.html) to explore different firing behaviors. The bottom of that page 
            has a list of parameter settings you can try, for example, "Tonic Bursting" or "Phasic Spiking".  Observe the different behaviors
            in the `Spikes' plot.
