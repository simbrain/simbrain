...
fun NetworkPanel.createConditionallyEnabledAction(
    ...
    initBlock = {
        ...
        updateAction()
        selectionManager.events.selection.on(Dispatchers.Swing) { _, _ -> updateAction() }
        selectionManager.events.sourceSelection.on(Dispatchers.Swing) { _, _ -> updateAction() }
...
fun createTooltipTextWithLocation(locatableModel: LocatableModel, convertToHtml: Boolean = true, stringSupplier: (LocatableModel) -> String = { it.toString() }) = """
        ...
        Location: ${locatableModel.location.format(2)}
        </html>
    """.trimIndent()
