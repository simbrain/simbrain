...
public class CreaturesBrain {
     ...
     * @param y
     * @param numNeurons
     * @param layoutName Valid input includes "line",
     * "vertical line", and "grid".
     * @param lobeName (optional)
     * @param neuronRule (optional)
     * @return
     */
    public NeuronGroup createLobe(double x, double y,
    		int numNeurons, String layoutName, String lobeName,
    		CreaturesNeuronRule neuronRule) {
    	NeuronGroup lobe = builder.addNeuronGroup(x, y, numNeurons,
    			layoutName, neuronRule);
    ...
    public NeuronGroup createLobe(double x, double y, 
    		int numNeurons, String layoutName, String lobeName) {
    	return createLobe(x, y, numNeurons, layoutName,
    			lobeName, new CreaturesNeuronRule());
    }
     ...
     */
    public NeuronGroup createLobe(double x, double y,
    		int numNeurons, String layoutName,
    		CreaturesNeuronRule neuronRule) {
    	return createLobe(x, y, numNeurons, layoutName,
    			"Lobe #" + lobes.size(), neuronRule);
    }
    ...
    */
     ...
     * @param name
     */
    public void nameNeuron(int lobeIndex, int neuronIndex, String name) {
    	nameNeuron(lobes.get(lobeIndex), neuronIndex, name);
    }

    /**
	 * Copies neuron labels from one neuron group 
	...
	public void copyLabels(NeuronGroup lobeToCopy, NeuronGroup lobeToPaste) {
		if (lobeToCopy.size() == lobeToPaste.size()) {
			for (int i = 0; i < lobeToCopy.size(); i++) {
				lobeToPaste.getNeuronList().get(i).setLabel(
						lobeToCopy.getNeuronList().get(i).getLabel());
			}
		} else {
			System.out.print(
					"copyToLabels error: Lobe " + lobeToPaste.getLabel()
					+ " is not the same size as Lobe " + lobeToCopy.getLabel());
		}
	}

	/**
	 * Manually gives a grid layout with a 
	 * set number of columns to a certain lobe.
	 * @param lobe
	 * @param numColumns
	 */
	public void setLobeColumns(NeuronGroup lobe, int numColumns) {
		GridLayout gridLayout = new GridLayout(
				gridSpace, gridSpace, numColumns);
