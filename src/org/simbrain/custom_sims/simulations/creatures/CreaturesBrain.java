	...
	public void copyLabels(NeuronGroup lobeToCopy, NeuronGroup lobeToPaste) {
		if (lobeToCopy.size() == lobeToPaste.size()) {
			for (int i = 0; i < lobeToCopy.size(); i++) {
				lobeToPaste.getNeuronList().get(i).setLabel(lobeToCopy.getNeuronList().get(i).getLabel());
			}
		} else {
			System.out.print("copyToLabels error: Lobe " + lobeToPaste.getLabel() + " is not the same size as Lobe "
					+ lobeToCopy.getLabel());
		}
	}
