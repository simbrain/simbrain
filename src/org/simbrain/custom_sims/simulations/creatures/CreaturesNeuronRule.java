...
public class CreaturesNeuronRule extends LinearRule {
	...
	/**
	 * The threshold at which the neuron will either fire (if state value is greater
	 ...
	 * 
	 */
	// TODO: Is it safe to have a per-neuron variable in this class, or should this
	// be moved down to the neuron level? Might be able to just use activation value
	// in lieu of this.
	// TODO: If we switch from using the state value variable to something else
	// (such as activation), change the javadoc comments in this class to match.
	private double stateValue;

	/**
	 * Set this value to TRUE to skip the update process. Good for neurons that are
	 * coupled as outputs.
	 */
	...
	public void update(Neuron neuron) {
		...
		if (!override) {
			if (stateValue > nominalThreshold) {
				...
				neuron.setBuffer(0);
				// TODO: Find an algorithm that better replicates how leakage rate in Creatures
