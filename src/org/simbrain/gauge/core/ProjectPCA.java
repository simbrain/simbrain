...
/**
 * <B>ProjectPCA</B> Projects the high-dimensional dataset along its
 * two principal components to the low-d dataset.
 */
		...
		for(int i = updim-1, k=0; i >= (updim - lowdim); i--) {
			...
			for(int j = 0; j < updim; j++) {
				if (combined.get(j,0) == val) {
					if (k >= lowdim) break; // needed for cases of repeated e-vals?  
