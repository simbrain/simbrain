...
public class PNodeNeuron extends PPath {
	...
	public static PNodeNeuron getDuplicate(PNodeNeuron toCopy, NetworkPanel np) {
		...
		ret.setNeuron(Neuron.getDuplicate(toCopy.getNeuron()));
		return ret;		
	}
	...
	private void createText() {
		...
		text = new PText(String.valueOf((int) Math.round(the_neuron.getActivation())));
		text.setFont(NEURON_FONT);
		setPosition();		
		
		this.addChild(text);

		in_label = new PText(the_neuron.getInputLabel());
		...
		out_label = new PText(the_neuron.getOutputLabel());
	...
	private void updateText() {
		...
		if (net_panel.getInOutMode() == true) {
			in_label.setText(the_neuron.getInputLabel());
			out_label.setText(the_neuron.getOutputLabel());
		}
		
		double act = the_neuron.getActivation();
		setPosition();
		text.setScale(1);
		...
		// Between 0 and 1
		} else if ((act > 0) && (the_neuron.getActivation() < 1)) {
	...
	private void applyColor() {
		...
		double activation = the_neuron.getActivation();
		...
		if (activation > -.1 && activation < .1) {
			...
			int hot_color = (int)(100 * (activation/the_neuron.getUpperBound()) + 155);
			hot_color = checkValid(hot_color);
			this.setPaint(new Color(hot_color,0,0));	
		} else if (activation < 0) {
			int cold_color = (int)(100 * Math.abs(activation/the_neuron.getLowerBound()) + 155);
	...
	public void setInput(boolean in) {
		...
		if (in == true) {
			if (the_neuron.getInputLabel().equals("not_input")) {
				the_neuron.setInputLabel("" + the_neuron.getParentNet().getLargestInputIndex() + 1 );
			}
			the_neuron.setInput(true);
		}
		
		if (in == false) {
			the_neuron.setInput(false);
		}	
		updateInArrow();
	...
	public void setOutput(boolean out) {
		...
		if (out == true) {
			if (the_neuron.getOutputLabel().equals("not_output")) {
				the_neuron.setOutputLabel(net_panel.getWorld().getRandomMovementCommand());
			}
			the_neuron.setOutput(true);
		}
		
		if (out == false) {
			the_neuron.setOutput(false);
		}	
	 ...
	 */
	public void upArrow() {
		the_neuron.incrementActivation();
		this.render();
	}
	...
	public void downArrow() {
		the_neuron.decrementActivation();
		this.render();
	}
