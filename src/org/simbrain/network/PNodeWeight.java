...
public class PNodeWeight extends PPath {
	...
	public PNodeWeight(PNodeNeuron source, PNodeNeuron target, Synapse weight) {
		this.source = source;
		this.target = target;
		the_weight = weight;
		updateRadius();
	...
	public PNodeWeight(PNodeNeuron source, PNodeNeuron target) {
		this.source = source;
		this.target = target;
		the_weight = new Synapse(source.getNeuron(), target.getNeuron());
		updateRadius();
		init();
	}

	public PNodeWeight(PNodeNeuron source, PNodeNeuron target, double val) {

		this.source = source;
		this.target = target;
		the_weight = new Synapse(source.getNeuron(), target.getNeuron(), val);
		updateRadius();
		init();
	}
	...
	public static PNodeWeight getDuplicate(PNodeWeight toCopy) {
		PNodeWeight ret = new PNodeWeight(toCopy.getSource(), toCopy.getTarget(), Synapse.getDuplicate(toCopy.getWeight()));
		return ret;		
	}
	...
	private void init() {
		...
		calColor(the_weight.getStrength(), isSelected());
	...
	public void updateRadius() {
		double str = the_weight.getStrength();
		
		if(str > 0) {
			radius = ((maxRadius - minRadius) * (str/the_weight.getUpperBound()) + minRadius);
		} else {
			radius = ((maxRadius - minRadius) * (Math.abs(str/the_weight.getLowerBound())) + minRadius);
		}
	}
