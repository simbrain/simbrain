...
public class Synapse {
	 ...
	 * reference (especially for combo-boxes) by GUI classes. TODO: Is this the
	 * best place for this? This is more efficient, but seems like it might
	 * violate the MVC paradigm.
	 */
	...
	public String toString() {
		...
		ret +=
				("  Connects neuron " + getSource().getId()
						+ " to neuron " + getTarget().getId() + "\n");
		return ret;
	}
	...
	public void setLearningRule(String name) {
		try {
			SynapseUpdateRule newRule =
					(SynapseUpdateRule) Class.forName(
	...
	public static Synapse getTemplateSynapse() {
		return new Synapse(null, null, new ClampedSynapseRule(),
				(Network) null);
	}
	 ...
	 * associated with a list of synapses. The list is compact in the sense that
	 * the size of the list is determined by the number of active update rule
	 * objects managing the synapses in the list. For instance, the list being
	 * queried may have 1000 synapses, but only 1 synapse update rule, since one
	 * synapse update rule object can service any number of synapses. In such a
	 * case this method would return a singleton list. This makes multiple
	 * consistency checks down the line much more efficient.
	 * 
	 ...
	 * @return Returns a compact list of synapse update rules associated with a
	 *         group of synapses
	 */
	 ...
	 *         properties
	 */
	public Synapse instantiateTemplateSynapse(Neuron source,
			Neuron target, Network parent) {
		this.source = source;
		this.target = target;
