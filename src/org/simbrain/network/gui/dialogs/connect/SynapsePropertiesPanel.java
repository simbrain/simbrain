...
public class SynapsePropertiesPanel extends JPanel {
	 ...
	 * 
	 * @param connection
	 *            the connection object which will be altered when values from
	 *            this panel are committed.
	 */
	...
	private void initializeLayout() {
		...
		exR.setBorder(BorderFactory.createLineBorder(new Color(255, 50,
				50)));
		this.add(exR, gbc);
		...
		gbc.gridy = 0;
		gbc.gridwidth = 2;
		inR.setBorder(BorderFactory.createLineBorder(new Color(50, 50,
				255)));
		this.add(inR, gbc);
	...
	private void initializeListeners() {
		...
		exSynListener = new SwitchableActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (selected != NULL_STRING && isEnabled()) {
					synapsePanel =
							AbstractSynapsePanel.RULE_MAP.get(exSynTypes
									.getSelectedItem());
					...
					if (!(synapsePanel instanceof StaticSynapsePanel)) {
						StandardDialog sdsp = new StandardDialog() {
							...
							protected void closeDialogOk() {
								super.closeDialogOk();
								synapsePanel
										.commitChanges(exTemplateSynapse);
							}
						};
		...
		inSynListener = new SwitchableActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (selected != NULL_STRING && isEnabled()) {
					synapsePanel =
							AbstractSynapsePanel.RULE_MAP.get(inSynTypes
									.getSelectedItem());
					synapsePanel.fillDefaultValues();
					if (!(synapsePanel instanceof StaticSynapsePanel)) {
						StandardDialog sdsp = new StandardDialog() {
							...
							protected void closeDialogOk() {
								super.closeDialogOk();
								synapsePanel
										.commitChanges(inTemplateSynapse);
							}
						};
						sdsp.setContentPane(synapsePanel);
						sdsp.pack();
		...
		randExListener = new SwitchablePropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent arg0) {
				if (Double.parseDouble(exRandPanel.getTfLowBound()
						.getText()) < 0.0) {
					exRandPanel.getTfLowBound().setText("0.0");
				}
				if (Double.parseDouble(exRandPanel.getTfUpBound()
						.getText()) < 0.0) {
		...
		randInListener = new SwitchablePropertyChangeListener() {
			...
			@Override
			public void propertyChange(PropertyChangeEvent arg0) {
				if (Double.parseDouble(inRandPanel.getTfUpBound()
						.getText()) > 0.0) {
					inRandPanel.getTfUpBound().setText("0.0");
				}
				if (Double.parseDouble(inRandPanel.getTfLowBound()
						.getText()) > 0.0) {
	 ...
	 * 
	 * @param connection
	 *            the connection from which field values are derived.
	 */
	...
	public void commitChanges() {
		...
		if (exRandCheck.isSelected())
			exRandPanel
					.commitRandom(connection.getExcitatoryRandomizer());
		if (inRandCheck.isSelected())
			inRandPanel
					.commitRandom(connection.getInhibitoryRandomizer());
	}
