...
public class PointNeuronRulePanel extends AbstractNeuronPanel {
	...
	private JComboBox cbOutputFunction = new JComboBox(
			new OutputFunction[] { OutputFunction.DISCRETE_SPIKING,
	...
	private static final PointNeuronRule prototypeRule =
			new PointNeuronRule();
	...
	public PointNeuronRulePanel() {
		...
		inputsTab
				.addItem("Time averaging time constant", tfTimeAveraging);
	...
	public void fillFieldValues(List<NeuronUpdateRule> ruleList) {
		...
		if (!NetworkUtils.isConsistent(ruleList, PointNeuronRule.class,
			...
			tfER.setText(Double.toString(neuronRef
					.getExcitatoryReversal()));
		...
		if (!NetworkUtils.isConsistent(ruleList, PointNeuronRule.class,
			...
			tfIR.setText(Double.toString(neuronRef
					.getInhibitoryReversal()));
		...
		if (!NetworkUtils.isConsistent(ruleList, PointNeuronRule.class,
			...
			cbOutputFunction.setSelectedIndex(cbOutputFunction
					.getItemCount());
		} else
			cbOutputFunction.setSelectedItem(neuronRef
					.getOutputFunction());
	...
	public void fillDefaultValues() {
		tfER.setText(Double.toString(prototypeRule
				.getExcitatoryReversal()));
		tfIR.setText(Double.toString(prototypeRule
				.getInhibitoryReversal()));
	...
	protected void writeValuesToRule(NeuronUpdateRule rule) {
		...
		PointNeuronRule neuronRef = (PointNeuronRule) rule;
		...
		if (!tfLC.getText().equals(NULL_STRING))
			neuronRef.setLeakConductance(Double.parseDouble(tfLC
					.getText()));

		// Output Function
		if (!cbOutputFunction.getSelectedItem().toString()
				.equals(NULL_STRING))
		...
		if (!tfThreshold.getText().equals(NULL_STRING))
			neuronRef.setThresholdPotential(Double
					.parseDouble(tfThreshold.getText()));
