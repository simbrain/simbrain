 ...
 *
 */
public class SynapseGroupNodeBidirectional extends PNode
implements PropertyChangeListener, SynapseGroupArrow { 
     ...
     */
    public SynapseGroupNodeBidirectional(final NetworkPanel networkPanel,
            final SynapseGroup synGroup1,
        ...
        srcGroupNode = (NeuronGroupNode) networkPanel.getObjectNodeMap()
                .get(srcNGroup);
        tarGroupNode = (NeuronGroupNode) networkPanel.getObjectNodeMap()
                .get(tarNGroup);
        ...
        this.addChild(synGroup1Box);
        this.addChild(synGroup2Box);

        srcGroupNode.addPropertyChangeListener(PNode.PROPERTY_BOUNDS, this);
        tarGroupNode.addPropertyChangeListener(PNode.PROPERTY_BOUNDS, this);
    ...
    /**
     * Checks to make sure that this node is being appropriately applied. Namely, 
     * that the target group of the first synapse group equals the source group
     * of the second synapse group and vice-versa.
     * @param group1
     * @param group2
     * @throws IllegalStateException if the synapse groups in question don't
     * actually qualify as bidirectional when taken together.
     */
    private void consistencyCheck(SynapseGroup group1, SynapseGroup group2)
            throws IllegalStateException {
        boolean state1Error = !(group1.getTargetNeuronGroup()
                .equals(group2.getSourceNeuronGroup()));
        boolean state2Error = !(group1.getSourceNeuronGroup()
                .equals(group2.getTargetNeuronGroup()));
    ...
    public void layout(Point2D src, Point2D tar) {
        ...
        Point2D ar1StPt = new Point2D.Float((float) (src.getX()
                + stOffset.getX()), (float) (src.getY() + stOffset.getY()));
        Point2D ar1EndPt = new Point2D.Float((float) (tar.getX()
                - endOffset.getX()), (float) (tar.getY() - endOffset.getY()));

        Point2D ar2StPt = new Point2D.Float((float) (tar.getX()
                + endOffset.getX()), (float) (tar.getY() + endOffset.getY()));
        Point2D ar2EndPt = new Point2D.Float((float) (src.getX()
                - stOffset.getX()), (float) (src.getY() - stOffset.getY()));
