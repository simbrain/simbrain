...
public class EchoStateNetwork extends Subnetwork {
	...
	public void buildNetwork() {
		...
		List<Neuron> inputLayerNeurons =
				initializeLayer(new LinearRule(), numInputs);
		...
		GridLayout gridLayout =
				new GridLayout(betweenNeuronInterval,
						betweenNeuronInterval,
						(int) Math.sqrt(numResNodes));
		gridLayout.layoutNeurons(reservoirLayerNeurons);
		reservoirLayer =
				new NeuronGroup(getParentNetwork(), reservoirLayerNeurons);
		addNeuronGroup(reservoirLayer);
		reservoirLayer.setLabel("Reservoir");
		NetworkLayoutManager.offsetNeuronGroup(inputLayer,
				reservoirLayer, Direction.NORTH, betweenLayerInterval);
		...
		NetworkLayoutManager.offsetNeuronGroup(reservoirLayer,
				outputLayer, Direction.NORTH, betweenLayerInterval);
	...
	public void connectLayers(Sparse inToRes, Sparse resRecurrent,
		...
		inToRes.setParameters(getParentNetwork(), inputLayer,
				reservoirLayer);
		resRecurrent.setParameters(getParentNetwork(), reservoirLayer,
				reservoirLayer);
		addSynapseGroup(connectNeuronGroups(inputLayer, reservoirLayer,
				inToRes));
		addSynapseGroup(connectNeuronGroups(reservoirLayer,
				reservoirLayer, resRecurrent));

		if (backWeights) {
			outToRes.setParameters(getParentNetwork(), outputLayer,
					reservoirLayer);
			addSynapseGroup(connectNeuronGroups(outputLayer,
					reservoirLayer, outToRes));
		}

		// Weights: reservoir layer to output layer
		AllToAll allToAll =
				new AllToAll(getParentNetwork(),
						reservoirLayer.getNeuronList(),
						outputLayer.getNeuronList());
	...
	private double[][] harvestData() {
		...
		// State matrix
		double[][] returnMatrix =
				new double[inputData.length][columnNumber];
	 ...
	 */
	public void setReservoirNeuronType(
			NeuronUpdateRule reservoirNeuronType) {
