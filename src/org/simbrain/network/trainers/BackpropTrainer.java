...
public class BackpropTrainer extends IterableTrainer {
	...
	public void apply() {
		...
		for (int row = 0; row < numRows; row++) {
			...
			for (Neuron neuron : biasDeltaMap.keySet()) {
				BiasedUpdateRule biasedNeuron =
						(BiasedUpdateRule) (neuron.getUpdateRule());
		...
		for (int i = layers.size() - 1; i > 0; i--) {
			...
			if (i == layers.size() - 1) {
				for (int j = 0; j < numOutputs; j++) {
					...
					double targetValue =
							network.getTrainingSet().getTargetData()[row][j];
					double outputError =
							targetValue - outputNeuron.getActivation();
				...
				for (Neuron hiddenLayerNeuron : layer) {
					...
					for (Synapse synapse : hiddenLayerNeuron.getFanOut()) {
						...
						if (errorMap.get(nextLayerNeuron) != null) {
							sumFanOutErrors +=
									(errorMap.get(nextLayerNeuron) * synapse
											.getStrength());
						}
					}
					// Compute errors and deltas for previous layer
					storeErrorAndDeltas(hiddenLayerNeuron,
							sumFanOutErrors);
				}
			}
		}
	...
	public static void test() {
		...
		// Layout object
		LineLayout layout =
				new LineLayout(50, LineOrientation.HORIZONTAL);
		...
		AllToAll connection =
				new AllToAll(network, inputLayer, hiddenLayer);
		connection.connectNeurons(true);

		// Connect hidden layer to hidden layer2
		AllToAll connection2 =
				new AllToAll(network, hiddenLayer, hiddenLayer2);
		connection2.connectNeurons(true);

		// Connect hidden layer2 to output layer
		AllToAll connection3 =
				new AllToAll(network, hiddenLayer2, outputLayer);
