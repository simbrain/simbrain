 ...
 * @author Zach Tosi
 *
 * A class which performs a parallelized update of an entire network. Neurons
 * are updated in chunks of ~100, and any given thread which is updating a 
 * neuron is also responsible for updating all the afferent synapses of that 
 * neuron. Update occurs synchronously using the same technique as buffered
 * update, despite being concurrent.
 * 
 * This class keeps its own separate list of neurons in the network because
 * for networks with groups, it would be expensive to extract neuron groups
 * from a generic groups list every invocation. Instead it implements a
 * group and neuron listener and updates its internal set of neurons when/if
 * neurons or neuron groups are added to or removed from the network. 
 * 
 ...
 * and execute them until the queue is empty. In this way, consumers can take
 * on tasks whenever they are available to do so and there are more tasks to
 * complete, instead of having to wait on some other condition. In order to
 * keep the sort of synchronization necessary for a neural network, a cyclic
 * barrier ensures that all tasks have been completed before more tasks 
 * (amounting to the next network iteration/update) are allowed in the queue. 
 * Furthermore this class implements a synchronous or buffered update. The
 * producer thread does not set the activation of each neuron to their buffer
...
public class ConcurrentBufferedUpdate implements NetworkUpdateAction,
	...
	/** 
	 * A list of threads being used as our "consumers" of tasks
	 * (updating neurons).
	 */
	...
	/** The barrier which prevents producer/consumer threads from moving on
	 *  until the entire network has been brought into the next time step.
	 */
	...
	private final Set<Neuron> neurons = Collections.synchronizedSet(
			new HashSet<Neuron>(INITIAL_CAPACITY));
	...
	/** 
	 * Indicating whether or not this class and consumer threads are dead
	 * and can no longer process network updates.
	 */
	 ...
	 * also not active at the same time at least half the time). Also
	 * populates this classes copy of the neurons in the network.
	 * 
	...
	public void invoke() {
		if (dead) {
			throw new IllegalStateException("ConcurrentBufferedUpdate cannot be"
	...
	/**
	 * Permanently shuts down the consumer threads and thus the functionality
	 * of this class.
	 */
	...
	public static void main(String[] args) {
		...
		SynapseGroup sg = SynapseGroup.createSynapseGroup(ng, ng,
				new Sparse(0.01, false, false), 0.75, exRand, inRand);
		net.addGroup(ng);
		net.addGroup(sg);
		...
		System.out.println("Serial: " + SimbrainMath.roundDouble((end - start)
				/ Math.pow(10, 9), 6));
		...
		System.out.println("Parallel: " + SimbrainMath.roundDouble((end - start)
				/ Math.pow(10, 9), 6));
