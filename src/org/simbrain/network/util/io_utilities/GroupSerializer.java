...
public class GroupSerializer {
	 ...
	 * using the least possible bytes see {@link #rowCompMat2CompByteArray(long[], Precision)}.
	 * @param synGrp the synapse group to serialize
	 * @param wtPrecision the precision (32 or 64 bit) used to store the weight
	 * values
	 * @param filename the name of the file containing the synapse group
	 *  information.
	 */
	 ...
	 * @param riCompressedMat row compressed matrix where column values are
	 * separated by a new row code (-1 or 0xffffffff) representing positions in
	 * a sparse matrix
    		...
    		if (numNonZero < riCompressedMat[0]) {
    			...
    			if (!switchedToShorts && !switchedToInts) {
    				if (riCompressedMat[i] > Byte.MAX_VALUE * 2) {
    					...
    					preByteArray.add((byte) (((int) riCompressedMat[i]) << 24
    							>>> 24));
    					numNonZero++;
    				}
    			}
