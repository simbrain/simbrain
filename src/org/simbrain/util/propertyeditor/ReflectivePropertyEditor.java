...
public class ReflectivePropertyEditor extends JPanel {
	 ...
	 * It's up to the user to embed it in a dialog or other GUI element
	 * 
	...
	/**
	 * Construct a property editor dialog. For use in a dialog box. 
	 * Formatted with an OK and Cancel button. 
	 * 
	 * @param toEdit the object to edit.
	 * @param parentDialog the parent dialog.
	 */
	public ReflectivePropertyEditor(final Object toEdit, final JDialog parentDialog) {
		this.toEdit = toEdit;
    ...
    private void initPanel() {
        for (final Method method : toEdit.getClass().getMethods()) {
			boolean inBaseClass = (method.getDeclaringClass() == toEdit.getClass());		
			if (inBaseClass) {
	...
	public void commit() {		
		for (Method m : editableMethods) {
			...
			// Colors
				...
				// This one's easy because the colors are already stored in a map
	...
	private boolean hasMatchingSetter(Method getter) {
		...
		for (Method possibleSetter : toEdit.getClass().getMethods()) {
			if (possibleSetter.getName().equalsIgnoreCase("set" + propertyName)) {
				...
				// System.out.println(possibleSetter.getParameterTypes()[0].getName() + " =? " + getter.getReturnType().getName());
				if (possibleSetter.getParameterTypes()[0].getName() == getter.getReturnType().getName()) {
					// At this point set the list of setter methods for use in commit
	...
	/**
	 * Format a string by inserting spaces and capitalizing.   This makes
	 * the item labels more appealing.
	 *
