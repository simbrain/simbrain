...
public class GameWorld2D extends StdGame {
    ...
    /** */
    public static final int PLAYERBLOCK_T = WALL_T | SHWALL_T | DOOR_T | GEN_T;
    /** */
    public static final int MONSTERBLOCK_T = WALL_T | SHWALL_T | DOOR_T | BONUS_T | MONSTER_T
            | GEN_T | BONUS_T | KEY_T | HEALTH_T;
    /** */
    public static final int BULLETBLOCK_T = WALL_T | DOOR_T | BONUS_T | BONUS_T | KEY_T | HEALTH_T;
    ...
    public void defineLevel() {
        ...
        for (int i = 0; i < nr_mushroomgen; i++) {
            ...
            new Generator(pos.x, pos.y, (int) random(mushroomrate, mushroomrate * 1.5), 0);
        }

        for (int i = 0; i < nr_demongen; i++) {
            Point pos = getTileCoord(getFreeTile());
            new Generator(pos.x, pos.y, (int) random(demonrate, demonrate * 1.5), demon_firerate);
        }

        for (int i = 0; i < nr_turrets; i++) {
            Point pos = getTileCoord(getFreeTile());
            new Turret(pos.x, pos.y, (int) random(turret_firerate, turret_firerate * 2.5));
        }
    ...
    public Point getFreeTile() {
        ...
        } while (and(getTileCid(x, y), 0x3FF) || (x < 6 && y < 6));
    ...
    public void digMazePath(int[][] cells, int x, int y, double openness) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            || x + xdir >= cells.length || y + ydir >= cells[0].length) {
                continue;
            }

            // is that cell already dug?
            if (and(cells[x + xdir][y + ydir], 16)
            && random(0.001, 1.0) > openness) {
                continue;
            }
            ...
            if (!and(getTileCid(getCenterTile(), 0, 0), MONSTER_T)) {
                ...
                if ((gencount--) <= 0) {
                    ...
                    if (monster_firerate == 0) { // gen mushrooms
                        if (nr_mushrooms < 80)
                            new StdDungeonMonster("mushroom", true, x, y, 4, "mushroom", false,
                                false, MONSTERBLOCK_T, MONSTER_T, 1.0, player, false, 0.0);
                    } else {
    ...
    public class Demon extends StdDungeonMonster {
        ...
        public Demon(double x, double y, int firerate) {
            super("demon", true, x, y, 4, "face", false, false, MONSTERBLOCK_T, MONSTER_T, 1.0,
                null, false, 0.0);
    ...
    public class Player extends StdDungeonPlayer {
        public Player(double x, double y, double speed, int upkey, int downkey, int leftkey, int rightkey) {
            super("player", x, y, 1, "human_l", false, false, PLAYERBLOCK_T, PLAYER_T, 2.2, upkey, downkey, leftkey, rightkey);
            stopAnim();
        }
