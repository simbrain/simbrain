 ...
 *
 * The diagram bakes theme colors into its shapes when it is drawn, so it has to be redrawn on a theme
 * switch rather than recolored in place. A correct dark result proves that redraw actually happens
 * through the live recolor hook, not just that the colors are right at construction time.
 */
