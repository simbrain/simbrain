...
class SupervisedModelTest {
    ...
    fun `simple supervised models should perform identically as backprop networks`() = runBlocking {
        ...
        (0 until 4).forEach { startingIndex ->
            ...
            backpropProbeDataCollector.keys.union(supervisedProbeDataCollector.keys).forEach { key ->
                val backpropValue = backpropProbeDataCollector[key] as? HashMap<*, *>
                val supervisedValue = supervisedProbeDataCollector[key] as? HashMap<*, *>
                if (backpropValue == null || supervisedValue == null) {
                    println("skipped $key")
                } else {
                    assertEquals(
                        backpropValue.entries.sortedBy { it.key.toString() }.map { it.value },
                        supervisedValue.entries.sortedBy { it.key.toString() }.map { it.value }
                    ) {
    ...
    @Test
    fun `simple supervised models with neuron group should perform identically as backprop networks`() = runBlocking {
        ...
        val layer1 = NeuronGroup(2).also { network2.addNetworkModels(it) }.also { it.isClamped = true }
        val layer2 = NeuronGroup(2).also { network2.addNetworkModels(it) }.also { it.updateRule = SigmoidalRule() }
        val layer3 = NeuronGroup(1).also { network2.addNetworkModels(it) }.also { it.updateRule = SigmoidalRule() }

        val wm1 = SynapseGroup(layer1, layer2).also { network2.addNetworkModels(it) }
        val wm2 = SynapseGroup(layer2, layer3).also { network2.addNetworkModels(it) }
