...
/**
 * <b>MouseEventHandler</b> was originally a PSelectionEventHandler but has a  reference to the NetworkPanel and
 * creates a differently colored marquis.  (PSelectionEventHandler only provided a black marquis, which is difficult
 * to see if the background color is also black ). TODO: Separate regular mouse handling from selection event handling
 *
...
public class MouseEventHandler extends PDragSequenceEventHandler {
    ...
    /**
     * Used in lasso selection. First,Êcalculate how many objects areÊcurrentlyÊin the lasso and add a rectangle to
     * represent a selected object.Ê Whenever the lassoÊgets changedÊdue to theÊ movement of the mouse ( computer
     * mouse, not the mice), updateMasquee updates the selection setÊto fit that change.Ê
     */
    protected void updatemarquis(PInputEvent pie) {
        ...
        marquis.setPathToRectangle((float) b.x, (float) b.y, (float) b.width, (float) b.height);
        b.reset();
        b.add(presspt);
        b.add(pie.getPosition());
    ...
    protected void computemarquisSelection(PInputEvent pie) {
        ...
        while (j.hasNext()) {
            ...
            if (!netPanel.getSelection().contains(node) && !marquisMap.containsKey(node)) {
            ...
            if (node instanceof PLayer) {
                for (Iterator i = netPanel.getSelection().iterator(); i.hasNext();) {
                    ...
                    if (n instanceof PCamera) {
                        if (((PCamera) n).indexOfLayer((PLayer) node) != -1) {
                            return true;
                        }
                    }
                }
    ...
    /**
     * If control key is pressed, add a new weight between last select neuron and current one.  If double-click, show
     * preference dialog.
     *
    ...
    public void mousePressed(PInputEvent e) {
        ...
        if (e.isControlDown() || (e.getButton() == 3)) {
            ...
            menu.show(netPanel, (int) e.getCanvasPosition().getX(), (int) e.getCanvasPosition().getY());
        }
    }
    ...
    public void mouseClicked(PInputEvent e) {
        ...
        if (netPanel.getMode() == NetworkPanel.ZOOMIN) {
            ...
            PBounds rec = new PBounds(
                                      e.getPosition().getX() - (VIEW / 2), e.getPosition().getY() - (VIEW / 2), VIEW,
                                      VIEW);
    ...
    private boolean canBeCopied(PNode node) {
        ...
        if (node instanceof PNodeWeight) {
            ...
            if (w.getSource() != null) {
                if (netPanel.getSelection().contains(w.getSource()) && netPanel.getSelection().contains(w.getTarget())) {
                    return true;
                }
            }
        }
