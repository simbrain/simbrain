...
public class RadialSimpleConstrainedKIn extends Sparse {
    ...
    public RadialSimpleConstrainedKIn(final int excitatoryKIN,
    		...
    		final int excitatoryRadius, final int inhibitoryRadius,
    ...
    public RadialSimpleConstrainedKIn(final int defactoKIN,
    	final int defactoRadius) {
        this(defactoKIN, defactoKIN, defactoKIN, defactoRadius, defactoRadius,
        		defactoRadius);
    }
	...
	public void connectNeurons(SynapseGroup synGroup) {
		for (Neuron tar : synGroup.getTargetNeurons()) {
			...
			List<Neuron> srcNeuronsInRange = synGroup.getSourceNeuronGroup()
					.getNeuronsInRadius(tar, radius);
