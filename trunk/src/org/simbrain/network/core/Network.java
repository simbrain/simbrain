...
public class Network {
	 ...
	 * @return distance
	 */
	public static double getDistance(final Neuron neuron1,
	 ...
	 */
	public SynapseGroup
			getSynapseGroup(NeuronGroup src, NeuronGroup targ) {
	 ...
	 */
	public void transferNeuronsToGroup(List<Neuron> list,
			NeuronGroup group) {
	...
	public static double
			round(final double value, final int decimalPlace) {
	...
	private int getTimeStepPrecision() {
		int logVal =
				(int) Math.round((Math.log(this.getTimeStep()) / LOG_10));
		if (logVal < 0) {
	...
	public void fireNeuronTypeChanged(final NeuronUpdateRule old,
		...
		for (NeuronListener listener : neuronListeners) {
			listener.neuronTypeChanged(new NetworkEvent<NeuronUpdateRule>(
					this, old, changed));
		}
	}
	...
	public void fireSynapseRemoved(final Synapse deleted) {
		for (SynapseListener listener : synapseListeners) {
			listener.synapseRemoved(new NetworkEvent<Synapse>(this,
					deleted));
		}
	}
	...
	public void fireSynapseChanged(final Synapse changed) {
		for (SynapseListener listener : synapseListeners) {
			listener.synapseChanged(new NetworkEvent<Synapse>(this,
					changed));
		}
	}
	...
	public void fireGroupChanged(final Group old, final Group changed,
		...
		for (GroupListener listener : groupListeners) {
			listener.groupChanged(new NetworkEvent<Group>(this, old,
	 ...
	 */
	public void setSynapseVisibilityThreshold(
			int synapseVisibilityThreshold) {
	...
	public void connectNeuronGroups(final NeuronGroup sng,
		...
		if (tng instanceof Competitive) {
			group =
					new SynapseGroupWithLearningRate(this, sng, tng,
							connection);
		} else {
