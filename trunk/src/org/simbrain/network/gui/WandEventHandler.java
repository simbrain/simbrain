...
final class WandEventHandler extends PDragSequenceEventHandler {
	...
	protected void drag(final PInputEvent event) {
		...
		Ellipse2D.Double ellipse =
				new Ellipse2D.Double(position.getX() - radius / 2,
						position.getY() - radius / 2, radius, radius);
		boundsFilter.setEllipse(ellipse);

		Collection highlightedNodes =
				networkPanel.getCanvas().getLayer().getRoot()
	...
	private void modifyNode(NeuronNode node) {
		...
		neuron.forceSetActivation(neuron.getUpdateRule().getCeiling());
		node.update();
	}
	...
	private class BoundsFilter implements PNodeFilter {
		...
		public boolean accept(final PNode node) {
			...
			boolean boundsIntersects =
					ellipse.intersects(node.getGlobalBounds());
	...
	private class ZoomEventFilter extends PInputEventFilter {
		...
		public boolean acceptsEvent(final PInputEvent event,
				final int type) {
