...
public class SynapseAdjustmentPanel extends JPanel {
	...
	private RandomPanelNetwork randomPanel =
			new RandomPanelNetwork(false);
		...
		randomizeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (Synapse synapse : networkPanel
						.getSelectedModelSynapses()) {
					if (synapseIsAdjustable(type, synapse)) {
						synapse.forceSetStrength(randomizer.getRandom());
					}
				}
				networkPanel.getNetwork().fireNetworkChanged();
			}
	...
	private void updateHistogram() {
		...
		// single row.
			...
			case "I/E Overlay": {
				...
				double[] hist1 =
						Arrays.copyOf(weights[0], weights[0].length);
				// Note: stored as absolute values...
				double[] hist2 =
						Arrays.copyOf(weights[1], weights[1].length);
			...
			case "Inhibitory Only": {
				...
				Color[] pallet =
						Arrays.copyOf(HistogramPanel.DEFAULT_PALLET,
	...
	private void updateStats() {
		...
		meanLabel.setText("Mean: "
				+ SimbrainMath.roundDouble(stats[0], 5));
		medianLabel.setText("Median "
				+ SimbrainMath.roundDouble(stats[1], 5));
		sdLabel.setText("Std. Dev: "
				+ SimbrainMath.roundDouble(stats[2], 5));
	...
	private class ScalerPanel extends LabelledItemPanel {
		...
		public ScalerPanel(final NetworkPanel networkPanel) {
			...
			increaseButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					double amount =
							Double.parseDouble(tfIncreaseDecrease
									.getText());
					for (Synapse synapse : networkPanel
							.getSelectedModelSynapses()) {
						if (synapseIsAdjustable(((String) synTypeSelector
								.getSelectedItem()), synapse)) {
							synapse.forceSetStrength(synapse
									.getStrength()
					...
					for (Synapse synapse : networkPanel
						...
						if (synapseIsAdjustable(((String) synTypeSelector
							...
							if (Math.abs(synapse.getStrength()) < threshold) {
								networkPanel.getNetwork().removeSynapse(
										synapse);
							}
						}
					}
	 ...
	 * 
	 * @param filterValue
	 *            value of the synapse type selector
	 * @param synapse
