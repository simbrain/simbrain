...
/**
 * Display preferences for regarding excitatory and inhibitory connections.
 *
 * @author ztosi
 * @author jyoshimi
 *
...
public class ExcitatoryInhibitoryPropertiesPanel extends JPanel {
     ...
     */
    private final JButton excitatorySynType = new JButton("ClampedSynapseRule");

    /**
     * A button opening a menu to select the desired type of inhibitory synapse.
     */
    private final JButton inhibitorySynType = new JButton("ClampedSynapseRule");
    ...
    public ExcitatoryInhibitoryPropertiesPanel(final ConnectNeurons connection) {
        ...
        connection.getInhibitoryRandomizer().setUpperBound(-0.1);
        connection.getInhibitoryRandomizer().setLowerBound(-1.0);
        connection.getExcitatoryRandomizer().setUpperBound(1.0);
        connection.getExcitatoryRandomizer().setLowerBound(0.1);
        fillFieldValues();
        initializeContent();
        initializeLayout();
    }
            ...
            public void actionPerformed(ActionEvent arg0) {
                if (arg0.getSource() == randInhib) {
                    if (randInhib.isSelected()) {
                        ...
                        randInButton.setEnabled(false);
                        connection.setEnableInhibitoryRandomization(false);
                    }
                }
            }
        });
            ...
            public void actionPerformed(ActionEvent arg0) {
                if (arg0.getSource() == randExcite) {
                    if (randExcite.isSelected()) {
                        ...
                        randExButton.setEnabled(false);
                        connection.setEnableExcitatoryRandomization(false);
                    }
                }
            }
        });
