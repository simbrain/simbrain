...
/**
 * The <b>SparsityAdjustmentPanel</b> is a sub-panel for other connection
 * panels which adjusts the sparsity of the connection. This panel includes
 * a text field for setting the sparsity as well as a slider and a text
 * field displaying how many efferents per source neuron there would be if
 * efferents are equalized. All of these are kept in sync. 
 * 
 * @author ztosi
 *
 */
public class SparsityAdjustmentPanel extends JPanel{
    ...
    /** 
     * The number of target neurons being connected to. Used for determining
     * the number of efferents per source neuron when efferents are equalized.
     */
    ...
    /**
     * Constructs a gui panel for adjusting the sparsity of a sparse 
     * connect neurons object, and initializes all appropriate listeners.
    ...
    /**
     * Checks if all values in the text fields and sliders are consistent.
     * Used to prevent infinite listener loops. 
     * TODO: Replace with switchable (Simbrain Utils) listeners/
     * investigate event source issue
    ...
    private boolean consistentValues(){
    	...
    	int spsVal = (int)(100 * Double.parseDouble(sparsity.getText()) /
    			numTargs);
    ...
    public void fillFieldValues() {
    	if (connection == null || !(connection instanceof Sparse)) {
        	...
        	synsPerSource.setValue(new Integer((int) (numTargs * ((Number)
        			Sparse.getDEFAULT_SPARSITY()).doubleValue())));
        	sparsitySlider.setValue((int)(Sparse.getDEFAULT_SPARSITY() * 100));
    	} else {
    		sparsity.setValue(connection.getSparsity());
        	synsPerSource.setValue(new Integer((int) (numTargs *
        			connection.getSparsity())));
	...
	public void setAllowSelfConnect(boolean allowSelfConnect) {
		...
		int nt = allowSelfConnect ? numTargs : numTargs - 1;
		int sps = (int) (((Number) sparsity.getValue())
                .doubleValue() * nt);
        synsPerSource.setValue(new Integer(sps));
	}
