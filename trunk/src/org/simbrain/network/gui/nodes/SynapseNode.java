...
public final class SynapseNode extends ScreenElement {
	 ...
	 *            target neuronmode
	 * @param synapse
	 *            the model synapse this PNode represents
	 */
	...
	public void updatePosition() {
		...
		if (isSelfConnection()) {
			synapseCenter =
					globalToLocal(new Point2D.Double(target.getCenter()
							.getX() + offset, target.getCenter().getY()
							+ offset));
		} else {
			synapseCenter =
					globalToLocal(calcCenter(source.getCenter(),
							target.getCenter()));
		}
		...
		if (circle == null) {
			circle =
					PPath.createEllipse(0, 0, (float) offset * 2,
							(float) offset * 2);
		...
		if (!isSelfConnection()) {
			line.reset();
			line.append(
					new Line2D.Double(globalToLocal(source.getCenter()),
							synapseCenter), false);
	...
	private PPath getLine(final Point2D center) {
		if (isSelfConnection()) {
			return new PPath(new Arc2D.Double(getX(), getY() - 7, 22, 15,
					1, 355, Arc2D.OPEN));
		} else {
	...
	public void updateColor() {
		...
		if (source.getNeuron().getUpdateRule() instanceof SpikingNeuronUpdateRule) {
			if (((SpikingNeuronUpdateRule) source.getNeuron()
	...
	public void updateDiameter() {
		...
		if (synapse.getStrength() == 0) {
			...
			diameter =
					(((NetworkGuiSettings.getMaxDiameter() - NetworkGuiSettings
							.getMinDiameter()) * (strength / upperBound) + NetworkGuiSettings
							.getMinDiameter()));
		} else {
			diameter =
					(((NetworkGuiSettings.getMaxDiameter() - NetworkGuiSettings
							.getMinDiameter()) * (Math.abs(strength
							/ lowerBound))) + NetworkGuiSettings
							.getMinDiameter());
		}
	...
	protected JPopupMenu getContextMenu() {
		...
		contextMenu
				.add(new SetSynapsePropertiesAction(getNetworkPanel()));
	...
	protected JDialog getPropertyDialog() {
		SynapseDialog dialog =
				new SynapseDialog(this.getNetworkPanel()
						.getSelectedSynapses());
		return dialog;
	}
	...
	public String toString() {
		...
		ret +=
				"SynapseNode: (" + this.getGlobalFullBounds().x + ")("
