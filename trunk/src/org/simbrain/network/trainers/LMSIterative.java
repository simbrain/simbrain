...
public class LMSIterative extends IterableTrainer {
	...
	public void apply() {
		...
		for (int row = 0; row < numRows; row++) {
			...
			for (int i = 0; i < numOutputs; i++) {
				...
				double targetValue =
						network.getTrainingSet().getTargetData()[row][i];
				...
				for (Synapse synapse : outputNeuron.getFanIn()) {
					double deltaW =
							(learningRate * error * synapse.getSource()
									.getActivation());
				...
				BiasedUpdateRule bias =
						(BiasedUpdateRule) outputNeuron.getUpdateRule();
	...
	public static void test2() {
		...
		AllToAll connection =
				new AllToAll(network, inputLayer, outputLayer);
		connection.connectNeurons(true);

		// Set initial weights (from an Emergent sim)
		Network.getSynapse(inputLayer.get(0), outputLayer.get(0))
				.setStrength(.352391);
		Network.getSynapse(inputLayer.get(1), outputLayer.get(0))
				.setStrength(.354468);
		Network.getSynapse(inputLayer.get(2), outputLayer.get(0))
				.setStrength(.338344);
		Network.getSynapse(inputLayer.get(3), outputLayer.get(0))
				.setStrength(.3593);
		Network.getSynapse(inputLayer.get(0), outputLayer.get(1))
				.setStrength(.561543);
		Network.getSynapse(inputLayer.get(1), outputLayer.get(1))
				.setStrength(.584706);
		Network.getSynapse(inputLayer.get(2), outputLayer.get(1))
				.setStrength(.355258);
		Network.getSynapse(inputLayer.get(3), outputLayer.get(1))
				.setStrength(.555266);
