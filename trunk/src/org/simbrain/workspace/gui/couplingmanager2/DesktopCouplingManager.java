...
public class DesktopCouplingManager extends JPanel implements ActionListener, MouseListener, MouseMotionListener {
    ...
    public DesktopCouplingManager(final SimbrainDesktop desktop, final GenericFrame frame) {
        ...
        consumerJList.setCellRenderer(new ConsumingAttributeCellRenderer());
        ...
        producerJList.setCellRenderer(new ProducingAttributeCellRenderer());
     ...
     * name.
     */
    private static class ConsumingAttributeCellRenderer extends DefaultListCellRenderer {
        ...
        public java.awt.Component getListCellRendererComponent(final JList list,
            ...
            ConsumingAttribute consumer = (ConsumingAttribute) object;
            renderer.setText(consumer.getAttributeDescription());
            return renderer;
       }
    }
    ...
    private void addConsumerContextMenu(final JList list) {
        list.addMouseListener(new MouseAdapter() {
            public void mouseClicked(final MouseEvent me) {
                ...
                ArrayList<ConsumingAttribute> selectedConsumers = new ArrayList<ConsumingAttribute>();
                if (list.getSelectedValues().length == 0) {
                    selectedConsumers.add((ConsumingAttribute) list.getSelectedValue());
                } else {
                    for (int i = 0; i < list.getSelectedValues().length; i++) {
                        selectedConsumers.add((ConsumingAttribute) list
                                .getSelectedValues()[i]);
                    }
                }
     ...
     * @return popup menu.
     */
    private JPopupMenu getConsumerAttributePopupMenu(final ArrayList<ConsumingAttribute> consumers) {
...
//        }
     ...
     * name.
     */
    private static class ProducingAttributeCellRenderer extends DefaultListCellRenderer {
        ...
        public java.awt.Component getListCellRendererComponent(final JList list, final Object object,
            ...
            DefaultListCellRenderer renderer = (DefaultListCellRenderer)
            super.getListCellRendererComponent(list, object, index, isSelected, cellHasFocus);
            ProducingAttribute producer = (ProducingAttribute) object;
           renderer.setText(producer.getAttributeDescription());
            return renderer;
       }
    }
    ...
    private void addProducerContextMenu(final JList list) {
        list.addMouseListener(new MouseAdapter() {
            public void mouseClicked(final MouseEvent me) {
                ...
                ArrayList<ProducingAttribute> selectedProducers = new ArrayList<ProducingAttribute>();
                if (list.getSelectedValues().length == 0) {
                    selectedProducers.add((ProducingAttribute) list.getSelectedValue());
                } else {
                    for (int i = 0; i < list.getSelectedValues().length; i++) {
                        selectedProducers.add((ProducingAttribute) list
                                .getSelectedValues()[i]);
                    }
                }
     ...
     * Returns a menu populated by attributes.
     *
     * @param producers that need menu.e
     * @return menu for list of producers.
     */
    private JPopupMenu getProducerAttributePopupMenu(final ArrayList<ProducingAttribute> producers) {
        final JPopupMenu popup = new JPopupMenu();
//        ProducingAttribute producer = producers.get(0);
//        if (producer != null) {
        ...
        if (component != null) {
            ...
            if (comboBox == consumerComboBox) {
                if (component.getConsumers() != null)  {
                    consumerJList.setModel(new GenericListModel<ConsumingAttribute>(component.getCustomListOfConsumingAttributes()));
                }
          } else if (comboBox == producerComboBox) {
              if (component.getProducers() != null) {
                  producerJList.setModel(new GenericListModel<ProducingAttribute>(component.getCustomListOfProducingAttributes()));
              }
          }
        }
    }
